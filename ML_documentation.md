...
class DeepSeekProcessor:
    ...
    def create_mindmap(text)             # Карта памяти
...
|-------|----------|---------------------|
| `summarize` | Краткий структурированный конспект | `processor.summarize(text)` |
...
| `mindmap` | Текстовая карта памяти | `processor.create_mindmap(text)` |
