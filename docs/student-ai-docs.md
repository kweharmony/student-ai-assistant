...
- 🎙 Транскрибировать аудиозаписи лекций (через OpenAI Whisper - локально)
- 📝 Редактировать текст в WYSIWYG редакторе
- 🤖 Обрабатывать текст через ML-модели (DeepSeek v3.2)
...
| **openai** | Клиент для DeepSeek API (OpenAI-compatible) |
| **OpenAI Whisper** | Локальная транскрибация аудио в текст |
| **PyTorch** | ML фреймворк для локальных моделей Whisper |
| **FFmpeg** | Обработка аудио/видео файлов |
| **DeepSeek v3.2** | LLM для обработки текста через VseLLM провайдер |
