...
ML модуль разделен на **три независимых компонента** с четкими обязанностями:
...
#### **3. Локальный фильтр нецензурной лексики**
- **Файлы:** `profanity_filter.py`
- **Цель:** Локальная фильтрация запрещённых слов без API запросов
- **Операции:** Удаление/замена нецензурной лексики, защита от блокировки API
...
#### `ml/profanity_filter.py` - 🚫 Локальный фильтр нецензурной лексики
**Назначение**: Локальная фильтрация запрещённых слов без API запросов

**Главный класс**:
```python
class ProfanityFilter:
    def __init__(self)          # Инициализация словарей запрещённых слов
    def filter_text(text)       # Фильтрация текста с заменой на [слово]
    def has_profanity(text)     # Проверка наличия нецензурной лексики
```

**Особенности**:
- ✅ Работает **полностью локально** (без API запросов)
- ✅ Поддерживает **русский и английский** языки
- ✅ Использует **регулярные выражения** для точного сопоставления
- ✅ Заменяет запрещённые слова на `[слово]`
- ✅ Применяется **автоматически** перед отправкой в AI API
- ✅ Предотвращает блокировку API из-за случайных ошибок транскрибации

**Зачем нужен**:
Whisper иногда ошибочно транскрибирует речь как нецензурные слова. Этот фильтр удаляет их локально **до** отправки в AI API, предотвращая блокировку запроса.
