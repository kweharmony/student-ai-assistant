...
**Устойчивость HTTP-сессии** ([worker/worker.py](../worker/worker.py)): все запросы
идут через единый `requests.Session` со специальной настройкой против «протухших»
keep-alive соединений (их тихо закрывает сервер/NAT во время простоя — иначе
опрос зависал до read-таймаута, а восстановление занимало 10–15 мин):
- `_build_session()` навешивает `HTTPAdapter` с `Retry` (авто-ретраи на свежем сокете)
  и кастомный `_KeepAliveHTTPAdapter`, включающий **TCP keepalive** (`SO_KEEPALIVE` +
  переносимые `TCP_KEEPIDLE/INTVL/CNT` — ОС заранее детектит мёртвый сокет).
- `_reset_session()` вызывается в `except` главного цикла при любой ошибке опроса:
  закрывает пул соединений и создаёт новый — следующий опрос идёт по свежему
  соединению (эффект как от ручного перезапуска воркера, но автоматически).

Управление через системный трей ([worker/tray_app.py](../worker/tray_app.py)):
вкл/выкл polling, счётчик задач, выбранное устройство. **Потоки:** единый
скрытый Tk-root живёт в главном потоке (его `mainloop`), иконка `pystray` и сам
воркер — в фоновых потоках; диалоги «Статус»/«Настройки» планируются на главный
поток через `root.after()` (нельзя создавать `tk.Tk()` в других потоках — это
роняло процесс на Windows). Сохранение настроек вызывает `worker.reload_config()`
(обновляет адрес и пересоздаёт сессию с новым ключом). Конфиг
...
| `lectures` | [LecturesSection.tsx](../src/components/account/LecturesSection.tsx) | `/api/lectures/*` (`notes`, `notes/generate`+polling, `apply-filter`, `re-transcribe`, `audio`); `/api/catalog/{my-lecture-statuses,material-requests,requests,publish}`; бейдж лимита `generation` ([QuotaBadge](../src/components/account/QuotaBadge.tsx)) + 429-обработка. Несколько генераций идут параллельно (список задач + один общий polling; одна задача на пару лекция+режим) — см. BUSINESS_LOGIC §7 |
...
- **Время в БД — наивный UTC.** Все `DateTime`-колонки — `TIMESTAMP WITHOUT TIME ZONE`,
  код везде использует `datetime.utcnow()`. Если в колонку попадает tz-aware datetime
  от клиента (ISO с `Z`), asyncpg падает с `DataError` → 500. Перед записью клиентского
  времени приводите к наивному UTC (`.astimezone(utc).replace(tzinfo=None)`) — так
  сделано для `subscription_expires_at` в [admin.py](../api/routers/admin.py).
...
- **Протухшие keep-alive у воркера**: при долгом простое сервер/NAT тихо закрывают
  HTTP-соединение, и переиспользование протухшего сокета из пула `requests.Session`
  вешало опрос до read-таймаута, а восстановление — на 10–15 мин. Лечится в
  [worker/worker.py](../worker/worker.py): авто-ретраи + TCP keepalive + сброс сессии
  (`_reset_session`) при ошибке опроса (см. §10). Если правите сетевой слой воркера —
  не возвращайте «вечную» сессию без этих защит.
